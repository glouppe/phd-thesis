...
retrieve all \textit{direct} interactions, variable importances might in fact
...
(and assumingly direct) interactions.
Alternatively, a promising strategy might also be directly look for \textit{strongly}
relevant variables, that is for variables $X_i$ such that $I(X_i;X_j|B) > 0$
for \textit{all} $B$.
In all cases, impurity misestimations effects
should also be mitigated by using an adequate stopping criterion. When input
variables are of different scale of measurements or vary in the number of
...
preferably be normalized (e.g., by $H(X_j)$) before their aggregation.
