...
by $\sum_{i=3}^N \frac{1}{i}$ when developping $S(N)$. In that case, we have
$T'(N) = \Omega(N \log N)$.
...
speedup is about $O(\log N)$, though the proofs of the theorems reveal that this bound is not tight and that the actual speedup is sub-logarithmic.
...
decision tree, where each leaf contains one sample from ${\cal L}$, and
