...
\item \textit{Best case.}\hfill\\
      The induction procedure is the most effecient
...
Let us  note that due to the end-cut preference of classical impurity
...
$f(n) = C^\prime(N)$, $a=2$, $b=2$ and $k=1$. Accordingly, $T'(N)=\Theta(N\log^{k+1} N)=\Theta(N\log^2 N)$ and $T(N) = \Theta(K N\log^2 N)$.
\end{proof}
...
\end{cases}
\end{equation}
Again, the second case of the Master Theorem applies, this time for $f(n)=C^\prime(N)$, $a=2$, $b=2$ and $k=0$.
...
T'(N) &\geq c_2 N \log N + c_1^\prime + T'(N-1) \nonumber \\
      ...
      &\geq c_1^\prime N + c_2 \sum_{i=1}^N i \nonumber \\
      &= c_1^\prime N + c_2 \frac{N(N+1)}{2} \nonumber \\
      &= \Omega(N^2).
\end{align}
...
tight. More specifically, given the fact that
\begin{equation}
\sum_{i=1}^N i \log i = \log(H(N)),
\end{equation}
where $H(N)$ is the hyperfactorial function, complexity
can be expressed tightly as $T(N) = \Theta(K \log(H(N)))$.
\end{proof}
...
For $c_2 K N \log N \leq C(N)\leq c_3 K N\log N$ (for all $N \geq 1$), the time complexity for building a decision
tree in the average case is $\Omega(K N \log N)$ and $O(K N \log^2 N)$.
\end{theorem}
...
In the same way, a lower bound on $T'(N)$ can be derived by assuming that $C'(N) = c_2 N \log N$
in the equations above and approximating $\sum_{i=2}^N \frac{1}{i} \log(i-1)$
by $\sum_{i=2}^N \frac{1}{i}$ when developping $S(N)$. In that case, we have
$T'(N) = \Omega(N \log N)$.

As a result, $T(N) = \Omega(KN\log N)$ and $O(KN \log^2 N)$ in the average case.
\end{proof}
...
speedup is about $O(\log N)$, though the proofs of the theorems reveal that this bound is not tight.
...
\begin{table}
    ...
    \hline
    \hline
    CART & $\Theta(pN\log^2 N)$ & $O(pN^2\log N)$ & $O(pN\log^2 N)$ \\
    Bagging & $\Theta(Mp\widetilde{N}\log^2 \widetilde{N})$ & $O(Mp\widetilde{N}^2\log \widetilde{N})$ & $O(Mp\widetilde{N}\log^2 \widetilde{N})$  \\
    RF & $\Theta(MK\widetilde{N}\log^2 \widetilde{N})$ & $O(MK\widetilde{N}^2\log \widetilde{N})$ & $O(MK\widetilde{N}\log^2 \widetilde{N})$  \\
