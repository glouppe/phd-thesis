...
decision trees, thereby producing accurate ensembles. In particular, we studied
that decision trees built with too much randomization lead to an increase of
bias which cannot be compensated by a reciprocal decrease of variance. In
Section~\ref{sec:ntrt} however, we have shown that masking effects due variable
selection also induce a bias with respect to variable importances, since it
forces some of the branches of never being built, and therefore some of the
conditioning sets $B \in {\cal P}(V)$ to never be taken into account.  In this
section, we revisit this problem and show that variable selection is in fact
not the only cause of bias.
...
%   - montrer cependant que l'effet s'atténue à mesure que N augmente
