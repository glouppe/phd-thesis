...
that decision trees built with too much randomization lead to an increase of
bias which cannot be compensated by a reciprocal decrease of variance. By
contrast, we also showed in Section~\ref{sec:ntrt} that when variable selection
is not totally random, masking effects induce a bias with respect to variable
importances, since it forces some of the branches to never be built, and
therefore some of the conditioning sets $B \in {\cal P}(V)$ to never be taken
into account.  In this section, we revisit this problem and show that variable
selection is in fact not the only cause of bias in variable importances.
...
close to $0$. More importantly, we also observe that the larger the cardinality of the variable
(i.e., the larger its number of unique values), the larger its importance. In
their study, \citet{strobl:2007b} argue that this bias is due to variable
...
argument does directly explain why similar trends are observed when no variable
selection is performed (i.e., for ID3 or for $K=1$) nor when cut-points are
chosen at random, independently of the cardinality of the variable (i.e., in
ETs).
...
%   - montrer cependant que l'effet s'atténue à mesure que N augmente (choisir une méthode, fixer max_depth)
...
%      noter que l'erreur est d'autant plus qu'on splitte de façon exhaustive
