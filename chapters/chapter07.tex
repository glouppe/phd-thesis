...
In conclusion, evaluating impurity on small samples lead to over-estimations
...
\begin{remark}{Encoding of $N$-ary variables to binary variables}
...
$b_j$ binary input variables $\{X_j^{m} | m=1,\dots,b_j\}$, each encoding for one the possible splits. If we
consider the binarized learning set ${\cal L}^\prime$, for which all $p$ the
input variables have been transformed into $\sum b_j$ binary variables, then
our theoretical framework for totally randomized trees and exhaustive splits can
be applied and Theorem~\ref{thm:imp} could possibly be adapted. The only difference lies in the way binary variables are drawn at
random:  instead of splitting randomly on one of the $\sum b_j$ variables,
first one of the $p$ original variables is drawn uniformly at random, second of
its $b_j$ binary variables is selected for splitting the current node.
In this setting, the importance of $X_j$ therefore amounts to sum of importances
$\sum \text{Imp}(X_j^m)$ of its binary variables.
\end{remark}
...
with success in a wide range of scientific applications.
...
\item \textit{Feature selection.} \hfill \\
    ...
    \item If the goal is to improve the accuracy, a good subset
          of variables can typically be found by tuning the value $\alpha$ so as to
    ...
    \item In some other applications, the objective is to identify variables
          ...
          In a finite setting however, bias due impurity misestimations (as previously discussed in Section~\ref{sec:7:bias:high})
          makes it more difficult to identify variables that are truly
          relevant since their importances might appear be lower than the
          importance measures of noisy or irrelevant variables. Yet, several options
...
\item \textit{Biomark identification.} \hfill \\
    \todo{}
