...
\begin{table}
    ...
    \hline
    \end{tabular}
    \caption{Toy classification on two input variables.}
    \label{table:simulation:bias:tree}
\end{table}

As an illustrative example, let us consider a toy classification problem
...
\begin{figure}
    \centering
    \includegraphics[scale=1.0]{figures/ch7_trees_ets.pdf}
    \caption{Extremely randomized trees buit from Table~\ref{table:simulation:bias:tree}. The two first trees are generated with probability $\tfrac{1}{4}$ while the third tree is generated with probability $\tfrac{1}{2}$.}
