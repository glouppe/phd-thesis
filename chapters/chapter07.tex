...
factors on the basis of their expression level, in order to reconstruct the
...
independent supervised learning problems.  For each input variable $X_j$ (for
$j=1,\dots,p$), GENIE3 considers the supervised learning task consisting in
prediction of the target variable $X_j$ from the remaining $p-1$ variables
$V^{-j}$. Using random forests for solving each of these $p$ problems,
variable importances can be derived and used as an indication of the putative
link with $X_j$. The larger the importance, the more likely the conditional
dependency with $X_j$. Putative links are then aggregated over all $p$
variables to provide a ranking of interactions from which the network can
be reconstructed.
