...
\item \textit{Data access.}\hfill\\
    Looking for the best split on the input variable $X_j$ and/or partitioning
    ${\cal L}_t$ on $X_j$ requires to iterate repeatedly over the node sample
    values $x_{i,j}$, which may comes at a non
    negligible cost if data is accessed randomly.  Due to CPU caching and
    pre-fetching effects however, the closer the values in memory, usually the lower it takes
    to access them. In our case, the cost of data access can therefore be minimized in
    two ways:
    \begin{itemize}
