...
\item \textit{Data access.}\hfill\\
    ...
    ${\cal L}_t$ on $X_j$ requires to repeatedly access  over the node sample
    values $x_{i,j}$ (i.e., over the column values if samples are represented
    as a two dimensional array), which may comes at a non negligible cost if data is
    ordered randomly.  Due to CPU caching and pre-fetching effects however, the
