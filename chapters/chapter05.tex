...
\item \textit{Data access.}\hfill\\
    ...
    not ordered properly.  Due to CPU caching and pre-fetching effects however, the
    closer the values in memory, usually the lower it takes to access them. In
    our case, the cost of data access can therefore be minimized in two ways:
...
\item \textit{Sort algorithm.}\hfill\\
    ...
    Finally, partitioning node samples ${\cal L}_t$ into subsets ${\cal
    L}_{t_L}$ and ${\cal L}_{t_R}$ not only makes them purer in terms of the
    ouput variable $Y$, it also usually makes node samples more and more
    identical in terms of the input variables $X_j$. As we get deeper into the
    tree, this property can be leveraged by implementing Quicksort as a
    three-way recursive partition~\citep{bentley:1993}, thereby setting  at their
    final positions, in a single recursive call, all the elements that are
    identical to the pivot  and ensuring good performance.
