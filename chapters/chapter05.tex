...
forests on artifical data. Our goal is to investigate whether the
...
For our experiments on simulated data, we consider the Friedman~\#1 artifical
...
variables, the number $K$ of variables drawn at each node and the effects of
using bootstrap replicates ${\cal L}^m$ instead of ${\cal L}$, all with respect to
...
on $N=1000$ learning samples of $p=10$ input variables, with $M=250$ trees and $K=\sqrt{N}$ set by
default. Statistics reported below are averages over 10 runs.
...
\caption{Effect of $M$ on building times, average depth and MSE.}
...
\caption{Effect of $N$ on building times, average depth and MSE.}
...
\caption{Effect of $p$ on building times, average depth and MSE.}
...
\caption{Effect of $K$ on building times, average depth and MSE}
\label{fig:5:artificial:K}
\end{figure}

Figure~\ref{fig:5:artificial:M} first illustrates the effect of the number $M$
of trees on the time required for building a forest (on the left), the average
depth of the trees (in the middle) and the error of the model (on the right).
As expected, the time complexity of the induction procedure is  $O(M)$
while the average depth of the leaves is constant with $M$ (since it has no
effect on the structure of the individual trees). The left plot also confirms
that ETs are faster to build than RF, even if trees in ETs are on average one
level deeper that trees in RFs, as shown in the middle plot. Finally, the mean
square error of the model shows to be inversely proportional to the number of
trees in the forest, which confirms our previous results from
...
finally illustrates that the more the data, the better the resulting model.
Interrestingly, it also shows that RF and ETs tend towards the same error rate as
...
splitting on noisy variables, partitions of the node samples are more likely to
be unbalanced rather than the opposite. This explains why on the middle plot
the average depth of RF eventually gets larger than the depth of ETs -- which
does not suffer from end-cut preference.
...
\caption{Effect of bootstrap replicates on building times, average depth and MSE.}
...
model when they are too strong (at $K=1$) but then become beneficial. For RF,
the optimal trade-off is at $K=5$ while the lowest error can be achieved in ETs
for $K=7$.
...
drawn with replacement from ${\cal L}$ is about $1.5$ faster than building
trees directly on ${\cal L}$, which is in accordance with our theoretical
results from Section~\ref{sec:5:complexity-fit}. The effect for ETs is less
strong, but still leads to a non-negligeable speedup. The average depth for RF
and ETs is about $0.5$ smaller when using bootstrap replicates. Again, this is
...
While not dicussed here, further experiments carried out on other artificial
data, for both regression and classification, lead to similar conclusions as
...
\begin{table}
    ...
    \centering
    \rotatebox{90}{
    \begin{tabular}{| c | p{1.5cm} >{\centering}p{0.8cm} p{1.8cm} p{2.5cm} >{\centering}p{1.8cm} >{\centering}p{0.8cm} >{\centering}p{0.8cm} >{\centering}p{1.3cm} p{3.5cm} |}
    \hline
...
requires a dedicated cloud environment. \todo{Suite...}


% décrire le protocol expérimental
...
% Table de speedups (avec Scikit-Learn-RF comme baseline, for all datasets + on average)
% - idem pour predict

% Comparison of avg depth / leaves for sklearn
