...
like TMVA or Orange, include more advanced alternatives like the gain ratio or
implement feature selection, e.g., using the Relief algorithm~\citep{kira:1992}. The
most common stopping criteria are an upper bound on the depth of the trees
(e.g., \texttt{max\_depth} in Scikit-Learn) along with a minimum number of
samples required for splitting a node (e.g., \texttt{nmin} in OK3). Some
packages also allow to set an upper limit on the number of nodes in the trees
or to stop the construction when a fixed accuracy is reached. Despite the
embarassing parallelism, only Scikit-Learn, Weka and H2O make it possible to
build random forests using multiple cores. Most notably, H2O builds on top of
Hadoop to provide a fully distributed implementation of random forests  (i.e.,
on a cluster). For performance reasons, implementations listed in
...
proprietary software (e.g., Salford Systems, which actually owns the
...
In the experiments made below, we benchmark the Scikit-Learn implementations
of RF and ETs against libraries listed in Table~\ref{table:implementations}.
\todo{Suite...}
