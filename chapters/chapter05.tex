...
Since the induction algorithm looks for splits that most decrease impurity, and
as $K \to p$, balanced partitions of the nodes samples should in fact be more
likely than partitions that are unbalanced. In practice, random forests should
therefore approach the best case rather than the average case defined above. As
$K \to 1$ however, and depending on the randomization, splits yield partitions that
tend to be more equiprobable, hence approaching the average case defined above.
As for the worst case, it should only arise in exceptional circumstances, when
the response $Y$ is hardly separable from the inputs $X$.
...
% RF should have better complexity than ET (in contrast to fit)

\begin{remark}{Distribution of randomized trees}
\todo{}
\end{remark}
