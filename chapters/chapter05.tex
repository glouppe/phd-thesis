...
respectively 75\% and 25\% of the original dataset as a learning and test sets,
$M=250$ trees and $K=\sqrt{p}$ set by default. Results reported below are
averages over 10 runs, each time resampling the learning and test sets but
using the same random folds accross all implementations.
...
as further illustrated in Figure~\ref{fig:5:benchmarks:mnist}). An explanation
...
Scikit-Learn implementation is again the fatest, far before all others. More
...
% on nique tout
% bias experimtnal du au choix des datasets
