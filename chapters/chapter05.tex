...
value of the impurity criterion can computed iteratively for consecutive
...
claim is however highly data dependent). In practice, random forests should
...
within polynimial time for all variants studied in this work. In the best
...
the mean squared error of the forest. Hyper-parameters are studied independently
...
\caption{Effect of the number $M$ (= \texttt{n\_estimators})  of trees ($N=1000$, $p=10$, $K=\sqrt{p}$).}
...
\caption{Effect of the size $N$ (= \texttt{n\_train}) of the learning set ($M=250$, $p=10$, $K=\sqrt{p}$).}
...
\caption{Effect of the number $p$ (= \texttt{n\_features}) of input variables ($N=1000$, $M=250$, $K=\sqrt{p}$).}
...
\caption{Effect of $K$ (= \texttt{max\_features}) in random variable selection ($N=1000$, $M=250$, $p=10$).}
...
\caption{Effect of bootstrap replicates ($N=1000$, $M=250$, $p=10$, $K=\sqrt{p}$).}
