...
\begin{table}
    ...
    \centering
    \rotatebox{90}{
    \begin{tabular}{| l | m{2cm} m{1.5cm} m{2.75cm} >{\centering}m{2cm} >{\centering}m{2cm} >{\centering}m{1cm} >{\centering}m{1cm} >{\centering}m{1.5cm} m{2cm} |}
    \hline
         & \textbf{Algorithms} & \textbf{Impurity} & \textbf{Stop} & \textbf{Class estimates} & \textbf{Variable importances} & \textbf{Multi-thread} & \textbf{Open source} & \textbf{Language} & \textbf{Reference} \\
    \hline
    \hline
    \textit{Scikit-Learn} & Bagging, RF, ETs, RS, RP & \texttt{"gini"} \texttt{"entropy"} \texttt{"mse"} & \texttt{max\_depth} \texttt{min\_samples\_split} \texttt{max\_leaf\_nodes} & $\checkmark$ & $\checkmark$ & $\checkmark$ & BSD3 & Python & \citep{pedregosa:2011} \\
    \textit{randomForest} &  &  &  & & & & & &  \\
    \textit{Weka} &  &  &  & & & & & &  \\
    \textit{OpenCV} &  &  &  & & & & & &  \\
    \textit{OK3} &  &  &  & & & & & &  \\
