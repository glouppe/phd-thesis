...
authors~\citep{toussaint:1974,stone:1978,nadeau:2003,hastie:2005,arlot:2010},
...
$\varphi$ learned on the entire set ${\cal L}$. As a result the estimates
...
$\varphi$ and estimating its generalization error.
...
In practice, $P(X, Y)$ is unknown and it is therefore not possible to derive
the Bayes model analytically. On simulated data however, where the distribution
is known, knowing $\varphi_B$ and comparing  $Err(\varphi_B)$ with the test set
error of a model $\varphi$ can be beneficial for evaluating how effective $\varphi$ is.
