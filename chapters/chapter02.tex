...
Accordingly, using an algorithm ${\cal A}$ for learning a model $\varphi_{\cal
...
% As introduced in Section~\ref{sec:2:model-selection}, classes of  learning
...
several hyperplanes that are in fact equally as good when evaluated in resubstitution. In
...
points is maximized, since in general the larger the margin the lower the
generalization error of the model.

Mathematically, support vector machines~\citep{boser:1992,cortes:1995} are maximum-margin linear models, of the form of
Equation~\ref{eqn:linear-model}, that are learned by solving the following
...
subject to
\begin{equation}
y_i(\mathbf{w}\cdot\mathbf{x_i} - b) \ge 1 - \xi_i, \quad \xi_i \ge 0.
\end{equation}
...
\end{equation}
such that ${\cal Y}=\{-1,1\}$ and where $C$ is an hyper-parameter that controls
...
The dual optimization problem is the same, except that the dot product
$\mathbf{x}_i\cdot\mathbf{x}_j$ is replaced by a \textit{kernel}
$K(\mathbf{x}_i, \mathbf{x}_j)$, which corresponds the dot product of
$\mathbf{x}_i$ and $\mathbf{x}_j$ in this new space.
