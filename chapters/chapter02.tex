...
search~\citep{bergstra:2012}). Finally, once $\widehat{\theta}^*$ is
identified, the learning algorithm is run once again on the entire learning set ${\cal
L}$, yielding the approximately optimal model ${\cal A}(\widehat{\theta}^*,
{\cal L})$.

Similarly, when ${\cal L}$ is scarse, the same procedure can be
carried out in the exact same way but using $K$-fold cross-validation estimates
(as defined by Equation~\ref{eqn:cv-error} of
ection~\ref{sec:estimating-generalization-error}) instead of test sample estimates.
