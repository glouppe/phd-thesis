...
The first and simplest estimate is the \textit{resubstitution estimate} or \textit{training sample estimate}. It
...
parts of $\theta$.} process yielding a model ${\cal A}(\theta, {\cal L}) =
...
use test sample estimates (as defined by Equation~\ref{eqn:test-error} of
Section~\ref{sec:estimating-generalization-error}) to guide the search
of the hyper-parameter values, that is:
\begin{equation}
\widehat{\theta}^* = \argmin_{\theta} \widehat{Err}_{{\cal L}_\text{train}}^{{\cal L}_\text{test}}({\cal A}(\theta, {\cal L}_\text{train}))
\end{equation}
In practice, solving this later equation is also a difficult task but approximations
can be obtained in several ways, e.g., using either manual tuning of
$\theta$, exhaustive exploration of the parameter space using grid search, or
dedicated optimization procedures (e.g., using random
search~\citep{bergstra:2012}). Similarly, when ${\cal L}$ is scarse, the same procedure can be
carried out in the exact same way but using $K$-fold cross-validation estimates
(as defined by Equation~\ref{eqn:cv-error} of
Section~\ref{sec:estimating-generalization-error}) instead of test sample estimates. In any case, once $\widehat{\theta}^*$ is
identified, the learning algorithm is run once again on the entire learning set ${\cal
L}$, finally yielding the approximately optimal model ${\cal A}(\widehat{\theta}^*,
{\cal L})$.
...
structures that are specific to the learning set. As Figure~\ref{fig:train-test-error}
...
\begin{figure}
    ...
    \caption{Training and test sample estimates with respect to the complexity
             of the model. The light blue curves show the training sample estimates over
             ${\cal L}_\text{train}$ (Equation~\ref{eqn:training-error})  while the light red curves show the test sample
             estimates  over ${\cal L}_\text{test}$ (Equation~\ref{eqn:test-error}) for $100$ learning sets ${\cal L} = {\cal
             L}_\text{train} \cup {\cal L}_\text{test}$ drawn at random from a
             known distribution. The thick blue curve is the expected generalization error
             $Err$ (Equation~\ref{eqn:expected-generalization-error}) and the thick red curve is the expected training sample estimate $\mathbb{E}_{\cal
             L} \{ \widehat{Err}_{\cal L}(\varphi) \} $. (Figure inspired from \citep{hastie:2005}.) }
...
A naive assessment of the generalization error of the selected model might be to
simply use the test sample estimate
$\smash{\widehat{Err}_{{\cal L}_\text{train}}^{{\cal L}_\text{test}}({\cal A}(\widehat{\theta}^*, {\cal L}_\text{train}))}$
that was minimized during model selection. The issue with this estimate
is that the learned model is not independent from ${\cal L}_\text{test}$
since its repeated construction was precisely guided by the minimization of its
prediction error over ${\cal L}_\text{test}$. As a result, the minimized
test sample estimate constitutes an underestimation of the true generalization
error, sometimes substantial. For the same reasons, using the $K$-fold cross-validation
estimate instead of the test sample estimate does not provide a better
estimate since model selection was similarly guided by the
...
procedure and only be used once the final model is selected. Appropriate and
correct protocols for both model selection and evaluation, using either test
samples estimates or $K$-fold cross-validation estimates, are respectively
given in Algorithm~\ref{algo:train-valid-test}, describing the
\textit{Train-Valid-Test set protocol}, and Algorithm~\ref{algo:cv}, describing the
\textit{nested $K$-fold cross-validation protocol}.
...
\item Perform model selection on ${\cal L}_\text{train}$ and ${\cal L}_\text{valid}$, i.e., find:
\begin{equation}
\widehat{\theta}^* = \argmin_{\theta} \widehat{Err}_{{\cal L}_\text{train}}^{{\cal L}_\text{valid}}({\cal A}(\theta, {\cal L}_\text{train}));
\end{equation}
\item Evaluate the generalization error of the selected final model as
\begin{equation}
\widehat{Err}_{{\cal L}_\text{train} \cup {\cal L}_\text{valid}}^{{\cal L}_\text{test}}({\cal A}(\widehat{\theta}^*, {\cal L}_\text{train} \cup {\cal L}_\text{valid}));
\end{equation}
...
\item For each fold $k=1, ..., K$:
    \begin{enumerate}
        \item Perform model selection on the subset ${\cal L}\setminus {\cal L}_k$ using nested $K$-fold estimates, i.e., find:
        \begin{equation}
            \widehat{\theta}_k^* = \argmin_{\theta} \widehat{Err}_{{\cal L}\setminus{\cal L}_k}^{CV}({\cal A}(\theta, {\cal L}\setminus{\cal L}_k));
        \end{equation}
        \item Evaluate the generalization error of the selected sub-model as
        \begin{equation}
            \widehat{Err}_{{\cal L}\setminus{\cal L}_k}^{{\cal L}_k}({\cal A}(\widehat{\theta}_k^*, {\cal L}\setminus{\cal L}_k);
        \end{equation}
    \end{enumerate}
\item Evaluate the generalization error the selected final model as the average generalization estimate of the model selected over the folds:
\begin{equation}
\frac{1}{K} \sum_{k=1}^K \widehat{Err}_{{\cal L}\setminus{\cal L}_k}^{{\cal L}_k}({\cal A}(\widehat{\theta}_k^*, {\cal L}\setminus{\cal L}_k)
\end{equation}
