...
algorithm, neural networks and finally tree-based methods on which this thesis
is about.
...
\textit{Nearest neighbor methods} form a class of learning algorithms known as
\textit{prototype methods}~\citep{hastie:2005}. They distinguish from other
learning algorithms in the sense that they are memory-based and require no
model to be fit.
...
neighbor algorithm~\citep{cover:1967} averages the output values from the $k$
...
choice. A notable variant of the $k$-nearest neighbor algorithm is the
...
proof of consistency of the method. When ${\cal L}$ grows to infinity and $k$
is appropriately adjusted to ${\cal L}$, the model produced the $k$-nearest
neighbor method is shown to converge towards the Bayes model.
