...
and estimating its generalization error. The test sample estimate of the generalization error of the model
$\varphi$ that would be obtained from ${\cal L}$ is then given as the
average prediction error over ${\cal L}_\text{test}$ of the model
$\varphi^\prime$ built on ${\cal L}_\text{train}$:
\begin{equation}\label{eqn:test-error}
\widehat{Err}_{{\cal L}_\text{train}}^{{\cal L}_\text{test}}(\varphi) = \frac{1}{N_\text{test}} \sum_{(\mathbf{x_i}, y_i) \in {\cal L}_\text{test}} L(y_i, \varphi^\prime(\mathbf{x_i}))
\end{equation}
...
generalization error of the model learned on the entire learning set.
...
to simply use the test sample estimate $\smash{\widehat{Err}_{{\cal
L}_\text{train}}^{{\cal L}_\text{test}}({\cal A}(\widehat{\theta}^*, {\cal
L}_\text{train}))}$ that was minimized during model selection. The issue with
this estimate is that the learned model is not independent from ${\cal
L}_\text{test}$ since its repeated construction was precisely guided by the
minimization of its prediction error over ${\cal L}_\text{test}$. As a result,
the minimized test sample error is in fact a biased, optimistic, estimate of
the true generalization error, sometimes leading substantial understimations.
For the same reasons, using the $K$-fold cross-validation estimate instead of
the test sample estimate does not provide a better estimate since model
...
\item For each fold $k=1, ..., K$:
    \begin{enumerate}
        ...
        \begin{equation}
            \widehat{\theta}_k^* = \argmin_{\theta} \frac{1}{K} \sum_{l=1}^K \widehat{Err}_{{\cal L}^{-k}\setminus{\cal L}_l^{-k}}^{{\cal L}_l^{-k}}({\cal A}(\theta, {\cal L}^{-k}\setminus{\cal L}_l^{-k}));
        \end{equation}
        ...
        \end{equation}
    \end{enumerate}
\item Evaluate the (unbiased) generalization error the selected model as the average generalization estimate of the sub-models selected over the folds:
\begin{equation}
...
\begin{equation}
    \widehat{\theta}^* = \argmin_{\theta} \frac{1}{K} \sum_{k=1}^K \widehat{Err}_{{\cal L}\setminus{\cal L}_k}^{{\cal L}_k}({\cal A}(\theta, {\cal L}\setminus{\cal L}_k));
\end{equation}
\item Learn the final model ${\cal A}(\widehat{\theta}^*, {\cal L})$ on the entire learning set.
\end{enumerate}
\end{algorithm}
