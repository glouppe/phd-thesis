...
arguments~\citep{geurts:2002}. Equation~\ref{eqn:exp-prediction-error} is also
...
${\cal L}$ but also the \textit{unseen} ones from ${\cal X}\times{\cal Y} \setminus
{\cal L}$.

For classification, the most common loss function is the \textit{zero-one}
loss function $L(Y, \varphi(X)) = 1(Y \neq \varphi(X))$, where all misclassifications
are equally penalized. In this case, the generalization error of $\varphi$
becomes the probability of misclassification of the model:
\begin{equation}
...
\subsection{Precision, recall and $F$-measure}
    ...
    % Find the model phi minimizing a loss L
    % Losses L
    % Estimation of L
        % restitution
        % train/test
        % cv
    % Bayes error, Bayes model
