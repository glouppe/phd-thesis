...
\item \citet{zhao:2000}:\hfill\\
    ...
    which therefore suffices to establish the strong consistency of the method when
...
\item \citet{breiman:2004}:\hfill\\
    ...
    to an (adaptative) nearest neighbor algorithm~\citep{lin:2006}, for which consistency
...
\item \citet{biau:2008}:\hfill\\
    ...
    or soft voting ensembles are also consistent. Note that this result
    ...
    in random order.  In this setting, and similarly to PERT, consistency of
    Purely Random Forests is guaranteed whenever $k^{(N)} \to \infty$ and
    $\tfrac{k^{(N)}}{N} \to 0$ as $N \to \infty$ (where $k^{(N)}$  is the value
    of $k$ for a forest grown on learning set of size $N$).
    ...
    this same work, and assuming no bootstrap sampling, the authors proceed and show that consistency is however
    ...
    along the chosen variabe) and then sampling uniformly within the gap.
...
\item \citet{biau:2012}:\hfill \\
    ...
    order until depth $\lceil \log_2 k \rceil$. Consistency is then guaranteed
    whenever  $p_j^{(N)} k^{(N)} \to \infty$ (for $j=1,\dots,p$) and
    $\tfrac{k^{(N)}}{N} \to 0$ as $N \to \infty$.

    In particular, this result can be shown to include the situation where, at
    each node, randomness is introduced by selecting at random a subset of $K$
    variables and splitting along the one that maximizes some impurity
    criterion, like in Random Forest. (Note however that best cut-points
    coincide with the mid-points only for some probability distributions.)
    Assuming no bootstrap sampling and provided that two independent datasets
    are used for evuating the goodness of the splits and fitting the prediction
    values at leafs, consistency of the method is then also established.

    Interestingly, this work also derives the result that performance
    of random forests only depend on the number $r$ of relevant variables,
    and not on $p$, making the method robust to overfitting.
...
\item \citet{denil:2013}:\hfill \\
    ...
    For each tree in the forest, the learning set is partioned randomly into
    ...
    In regression, assuming not bootstrap sampling, consistency of this variant
...
\item \citet{scornet:2014}:\hfill \\
    ...
    Forest algorithm. In particular, consistency is obtained in the context of
    regression additive models, assuming subsampling without replacement
    (instead of bootstrap sampling).

\end{description}

Together, these theoretical works provide converging arguments all indicating
why random forest methods work so well in practice.

In addition, let us finally complete this review by mentioning consistency
results in the case of domain-specific adaptations of random forests, including
quantile regression~\citep{meinshausen:2006}, survival
analysis~\citep{ishwaran:2010} and online forest construction~\citep{denil:2013b}.
