...
\item \citet{zhao:2000}:\hfill\\
    ...
    consistency  provided the construction of the trees stops early. More
    specifically, strong consistency is guaranteed as soon as trees are grown
    infinitely deeply while making leaves count infinitely many node samples
    (e.g., by stopping the construction when $N_t < N_\text{min}$, such that
    $N_\text{min} \to 0$ and  $N\times N_\text{min} \to \infty$ as $N \to
    \infty$), hence making empirical class proportions in terminal nodes
    converge towards their theoretical counterparts.

    \todo{Close connection with ETs?}
