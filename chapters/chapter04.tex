...
\item \citet{breiman:1996b}: \hfill \\
    ...
    L}^b \}$ form replicates of ${\cal L}$, each consisting of $N$ cases, drawn
    ...
    \end{equation}
    Bagging generates individual models that  are
    different from one bootstrap sample to another, hence making them likely to
    benefit from the averaging process. In some cases however, when the
    learning set ${\cal L}$ is small, subsampling $67\%$ of the objects might
    lead to an increase of bias which is too large to be compasented by a
    decrease of variance, hence resulting in overall poorer performance.
    Despite this defect, bagging has proven to be an effective method
    in numerous applications, one of its strengths being that it can be
    used to combine any kind of models -- i.e., not only decision trees.
...
\item \citet{amit:1997}: \hfill \\
    ...
    individually good. As result, bias usually only slightly increases, while
    variance due to randomization can be cancelled out through averaging.

\item \citet{ho:1998}: \hfill \\
    The \textit{random subspace} method combines the principles of
    bagging~\citep{breiman:1996b} with the idea of random subsets of
    variables~\citep{amit:1997}. More specifically, \citet{ho:1998} proposes to
    grow decision trees with the usual non-randomized induction procedure, but
    on random subsets of the input variables (drawn prior to the construction
    of each tree), rather than on all $p$ input variables. As empirically
    evaluated at several occasions~\citep{ho:1998,panov:2007,louppe:2012}, the
    random subspace is a powerful ensemble method that can achieve near
    state-of-the-art performance on many problems. As for bagging, one of the strengths
    of the method is its genericity -- i.e., it can in fact be used with any kind of models.

\item \citet{breiman:1999}: \hfill \\

\item \citet{dietterich:2000}: \hfill \\
...
% (Kwok & carter (1990)
% bagging 1996
% amit and german 1997
% dietterich 1998 (see rf)
% RF breiman 2001
% geurts 2002
% geurts 2006
