...
\item \citet{cutler:2001}: \hfill \\
    ...
    while the cut-point $v$ is set inbetween two randomly drawn samples using
...
\item \citet{rodriguez:2006}: \hfill \\
    ...
    predictions of the constituents of the ensemlbe), the input variables are
    then randomly split into $q$  subsets, principal component analysis (PCA)
    is run separately on each subset, and a new set of $p$ extracted input
    ...
    results that are often as good, sometimes better, than Random Forests.
