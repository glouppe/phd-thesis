...
bias, for which averaging is not able to compensate. For Random Forests, the
optimal trade-off is at $K=8$. For Extremely Randomized Trees, the
optimal trade-off is at $K=10$, suggesting that random thresholds $v$ provide
by themselves enough randomization on this problem.
...
that Extremely Randomized Trees are inherently less correlated than trees built
in a Random Forest, which is not surprising given the fact that the former
...
increase of bias, as in this problem, Extremely Randomized Trees are therefore
expected to yield better results than Random Forests. (The choice of the
optimal randomization strategy is however highly dependent on the problem
and no general conclusion should be drawn from this toy regression problem.)

\begin{figure}
    \centering
    \includegraphics[width=0.9\textwidth]{figures/ch4_variance.pdf}
    \caption{Decomposition of variance with respect to the number $M$ of trees in the ensemble.}
    \label{fig:variance}
\end{figure}

As expected, variance also additively decomposes  into
\begin{equation}
var(\mathbf{x}) = \rho(\mathbf{x}) \sigma^2_{{\cal L},\theta} + \tfrac{1 - \rho(\mathbf{x})}{M} \sigma^2_{{\cal L},\theta},
\end{equation} as confirmed by Figure~\ref{fig:variance} for the variance of
Random Forests. The first term is the variance due to the learning set ${\cal
L}$ and remains constant as the number $M$ of trees increases. The second term
is the variance due to random effects and decreases as $M$ increases.
