...
\item \citet{cutler:2001}: \hfill \\
    ...
    while the cut-point $v$ is set inbetween two randomly drawn samples, instead of using
    Algorithm~\ref{algo:findsplit:x_j} for finding the optimal threshold on
    $X_j$:
    \begin{algorithm}
...
\item \citet{geurts:2006}: \hfill \\
    ...
    selection~\citep{amit:1997} with random discretization
    thresholds. Effectively replacing Algorithm~\ref{algo:findsplit:x_j}, the
    following simplistic but surprisingly effective randomized variant
    is used for finding a random threshold:
    \begin{algorithm}
    Draw a random split on $X_j$ that partitions ${\cal L}_t$.
    \textnormal{
    \begin{algorithmic}[1]
...
\subsection{Illustration on a regression problem}
