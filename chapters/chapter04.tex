...
\item \citet{biau:2012}:\hfill \\
    ...
    are used for evaluating the goodness of the splits and fitting the prediction
    values at leafs, consistency of the method is then also established.

    Interestingly, this work also highlights the fact that performance of
    random forests only depend on the number $r$ of relevant variables, and not
    on $p$, making the method robust to overfitting.
...
confirming why random forest methods -- like Random Forest but also Extremely
Randomized Trees -- appear to work so well in practice.
