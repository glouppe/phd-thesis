...
ant its variants and study how randomness introduced into the decision trees
...
On this toy problem, we compare the bias-variance decomposition of the expected
generalization error of a Random Forest~\citep{breiman:2001} and of Extremely
Randomized Trees~\citep{geurts:2006}, as estimated from Theorem~\ref{thm:bias-variance:ensemble}
using $100$ randomly drawn learning sets ${\cal L}$ of $50$
samples, on which ensembles of $10$ trees are built. Their generalization error
is estimated on an independent test set of $1000$ samples.
...
As confirmed by Figure~\ref{fig:variance} for Random Forests, variance also additively decomposes  into
\begin{equation}
\text{var}(\mathbf{x}) = \rho(\mathbf{x}) \sigma^2_{{\cal L},\theta} + \tfrac{1 - \rho(\mathbf{x})}{M} \sigma^2_{{\cal L},\theta}.
\end{equation}
