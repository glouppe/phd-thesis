...
are still not well understood. In particular, the fundamental theoretical
question of the consistency of random forests, i.e., whether convergence
towards an optimal model is guaranteed provided an infinitely large learning
set, remains an open and difficult problem. In this section, we review
theoretical works that have investigated simplified versions of the algorithm,
for which the theoretical analysis is typically more tractable. With the hope
that results obtained for simplified models will provide insights on the
mathematical properties of actual random forests, the long-term objective of
this line of research is usually to prove the consistency of the particular and
well-established Random Forest algorithm~\citep{breiman:2001}.

\begin{description}

\item \citet{zhao:2000}:\hfill\\

\item \citet{lin:2006}:\hfill\\
