...
the predictions of these models into a new \textit{ensemble} model,
such that the expected generalization error of the ensemble is (hopefully)
smaller than the expected generalization error of any of the randomized models.

In regression, the most common way to combine the randomized models into an
ensemble, denoted $\psi_{{\cal L},\theta_1,\dots,\theta_M}$, is to average
their predictions to form the final prediction:
\begin{equation}
\psi_{{\cal L},\theta_1,\dots,\theta_M}(\mathbf{x}) = \frac{1}{M} \sum_{m=1}^M \varphi_{{\cal L},\theta_m}(\mathbf{x})
\end{equation}

In classification, ...
% citer breiman: vote vs average of estimates
