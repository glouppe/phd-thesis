...
\item \citet{cutler:2001}: \hfill \\
    ...
    variable to split on and the discretization threshold are chosen at random.
    Given a node $t$, the split variable $X_j$ is drawn at random using
    Algorithm~\ref{algo:findsplit:random} with $K=1$. Let $(\mathbf{x}_1,y_1)$ and
    $(\mathbf{x}_2, y_2)$ be two randomly drawn samples in $t$ such that $y_1 \neq
    y_2$. Then, the random decision threshold is
    \begin{equation}
    v = \alpha x_{1,j} + (1 -\alpha) x_{2, j},
    \end{equation} where $\alpha$ is drawn uniformly at random in $[0;1]$.
    The induction of the trees proceed using such random splits until
    ...
    trees that are typically deeper and larger than  decision trees grown with
    less randomization. The simplicity of the method also allows for an amenable
    theoretical analysis of random forests, as carried out in \citep{zhao:2000}.
...
\item \citet{geurts:2006}: \hfill \\
    ...
    ensembles~\citep{cutler:2001} since both draw $X_j$ and $v$ at random. Both
    methods are however not equivalent since they do not draw the random
    discretization threholds $v$ with respect to the same probability
    distribution.
