...
$\mathbf{x}$ into the (one-hot-encoded) indices of the leaves it ends up in. That is,
$\mathbf{x}^\prime_t$ is $1$ for all leaves $t$ of the forest in which $\mathbf{x}$ falls in,
and $0$ for all the others. In this formalism, the proximity between
$\mathbf{x}_1$ and $\mathbf{x}_2$ can therefore be restated as their normalized
dot product in ${\cal X}^\prime$:
\begin{equation}
...
linear methods, even when trees in the forest are built totally at
