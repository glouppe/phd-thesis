...
$\varphi_{\cal L}(\mathbf{x})$ in analogy with Equation~\ref{eqn:4:decomp1} for
the square error loss. (Note that, to simplify notations, we assume that all probabilities
based on the random variable $Y$ is with respect to the distribution of $Y$ at $X=\mathbf{x}$.)
\begin{align}
& \mathbb{E}_{\cal L}\{ \mathbb{E}_{Y|X=\mathbf{x}} \{ 1(\varphi_{\cal L}(x) \neq Y) \} \}  \\
...
\end{equation}
Probability~\ref{eqn:4:prob-diff-from-bayes} therefore corresponds to the tail
area of the estimate distribution on the left side of the decision threshold
at $0.5$.
