...
In typical real-world applications, data are often imperfect and contain
errors, uncertainties or missing values. In particular, most machine learning
algorithms are often not applicable on data with missing values because they
implicitely assume an ideal scenario in which all values are known for all
input variables. Fortunately, random forests offer several mechanisms for
...
\item \textit{Ternary decision trees.}\hfill\\
    ...
    and $t_R$ are defined from a binary split $X_t \leq v$, for all node
    samples where the value of $X_t$ is known, and such that $t_M$ contain all node
    samples for which the value of $X_t$ is missing.
...
\item \textit{Imputation.}\hfill\\
    ...
    values. The first approach, due to \citep{breiman:2002}, consists first in
