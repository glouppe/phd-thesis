...
\textit{proximity measure} between two sample points. Formally, in
classification, the proximity between $(\mathbf{x}_1, y_1)$ and $(\mathbf{x}_2,
y_2)$ is defined as the number of times both samples reach the same leaf $t$ in
all decision trees within the ensemble, normalized by the number $M$ of trees.
The idea is that even though the data may be high-dimensional or involving
mixed input variables, the proximity measure gives an indication of how close
the  samples are with respect to the random forest~\citep{hastie:2005}. When
proximity is close to $1$, samples propagate into the same leafs and are
therefore similar according to the forest. On the other hand, when it is close
to $0$, samples reach different leafs, suggesting that they structurally
