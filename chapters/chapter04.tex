...
\item \citet{breiman:1996b}: \hfill \\
    ...
    models built from \textit{bootstrap samples}~\citep{efron:1979} ${\cal L}^b$ (for
    $b=1,\dots,M$) of the learning set ${\cal L}$. The $\{ {\cal L}^b \}$ form
    replicates of ${\cal L}$, each consisting of $N$ cases, drawn at random but
    \textit{with replacement} from ${\cal L}$. Note that even though $|{\cal L}|=|{\cal
    L}^b|=N$, $37\%$ of the couples from ${\cal L}$ are on average missing in
    the bootstrap replicates. Indeed, after $N$ draws with replacement, the probability
    of never be selected is
    \begin{equation}
