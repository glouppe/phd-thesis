...
For the square error loss, the bias-variance decomposition of the expected generalization error
...
\item When the expected probability estimate $\mathbb{E}_{\cal L}\{ p_{\cal L}(Y=\varphi_B(\mathbf{x}) \}$
      ...
      value whatever the regression bias of the estimate.
...
Formally, results from sections~\ref{sec:bias-variance:regression} and \ref{sec:bias-variance:classification} can readily
