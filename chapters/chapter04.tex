...
&= \rho(\mathbf{x}) \sigma^2_{{\cal L},\theta} + \frac{1 - \rho(\mathbf{x})}{M} \sigma^2_{{\cal L},\theta}
\end{align}
As the size of the ensemble gets arbitrarily large, i.e., as $M \to \infty$,
the variance of the ensemble therefore reduces to $\rho(\mathbf{x})
...
% discussion increase of bias to allow for variance reduction. the crux of the problme is in finding the right trade-off.
