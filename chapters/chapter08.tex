...
Let us first consider and analyze the sets $\{(\alpha_s, \alpha_f,
Acc_D(\alpha_s,\alpha_f)) | \forall \alpha_s, \alpha_f\}$ for various problems, where $Acc_D(\alpha_s,\alpha_f)$ is the average
test accuracy of an ensemble built on random patches of size $\alpha_s \alpha_f$ (using
the same protocol as previously) on the dataset $D$.

As Figure~\ref{fig:9:surfaces} illustrates for six datasets, the
surfaces defined by these sets vary significantly from one problem to
...
Figure~\ref{fig:surfaces} with those in the 2$^{nd}$ and 4$^{th}$ rows). The only
...
low. Building base estimators on re-sampled random patches thus brings a clear advantage to RP, RS and P and hence confirms the conclusions of Basilico
et al who showed in~\cite{basilico:2011} that using more data indeed produces
