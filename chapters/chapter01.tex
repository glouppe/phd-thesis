...
\begin{itemize}
\item \todo{Explicit contributions}
\item \todo{Motivate tree-based methods (why not others?).}
\item \todo{Research on RF seems to have flattened in comparison to other fields. Why so? Is there anything left to discover?}
\end{itemize}
...
forest of randomized trees and study how these scores can be interpreted in
order to reveal the underlying predictive structure learned from the data. In
particular, we derive a theoretical framework from which we prove theoretical
...
some successful applications. Finally, Part~\textsc{\ref{part:3}}
