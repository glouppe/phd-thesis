...
roughly restated as finding the best partition of the node samples.

For a finite set ${\cal L}_t$ of samples, the number of partitions into $k$
...
As formulated in CART~\citep{breiman:1984}, the usual family ${\cal Q}$
of splits is the set of binary splits defined on a single variable and resulting in non-empty
subsets of ${\cal L}_t$:
...
\begin{equation}\label{eqn:q:ordered}
...
${\cal X}$ with axis-parallel hyperplanes, as illustrated previously on Figure~\ref{fig:3:partition}.
...
While Equations~\ref{eqn:q:ordered} and \ref{eqn:q:categorical-cart} certainly
constitute the most common types of splits, alternatives proposed over the
years in the literature are worth mentioning. In ID3, \citet{quinlan:1986}
replaces binary splits on categorical variables with exhaustive splits. That
is, if ${\cal X}_j$ counts $k$ different values, then a split on $X_j$ divides
${\cal X}_t$ into $k$ child nodes -- one for each value of the variable. In
oblique decision trees, \citet{heath:1993} propose to replace axis-parallel
cutting hyperplanes with hyperplanes of any orientation, resulting into
smaller, yet as accurate, decision trees.
