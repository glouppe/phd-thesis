...
approximated using estimates of $\mathbb{E}_{X, Y|t} \{ Y \}$. The simplest
approximation consists in using the average value of $Y$ for subset of learning
samples falling into $t$:
\begin{align}\label{eqn:assignement-regression-approx}
\widehat{y}_t &= \frac{1}{N_t} \sum_{\mathbf{x}_i, y_i \in {\cal L}_t} y_i
\end{align}
...
L}_t} (y_i - \widehat{y}_t)^2$, one can show that
