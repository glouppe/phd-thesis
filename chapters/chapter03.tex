...
\widehat{Err}_{\cal L}(\varphi) &= \sum_{t \in \widetilde{\varphi}} p(t) (1 - p(\widehat{y}_t | t)) \nonumber \\
    ...
    &= \frac{1}{N} \sum_{\mathbf{x}_i, y_i \in {\cal L}} 1(y_i \neq \varphi(t)) = \widehat{Err}_{\cal L}^\text{train}(\varphi)
\end{align}
In other words, minimizing Equation~\ref{eqn:assignment-generalization} (for a fixed structure) through
probability estimates computed from class proportions in ${\cal L}$ is in fact
equivalent to minimizing the resubstitution estimate of $\varphi$ (Equation~\ref{eqn:training-error}).
