...
{\cal Q}(X_j) = \{ (\{\mathbf{x} | x_j \in {\cal B} \}, \{\mathbf{x} | x_j \in \overline{\cal B}\}) | &{\cal B} \in {\cal P}(\{ b_1, \dots, b_L \}), \nonumber \\
                                                                                                      &1 \leq |{\cal B}| < L \}
\end{align}
where ${\cal P}(\{ b_1, \dots, b_L \})$ is the power set of $\{ b_1, \dots, b_L \}$ and $\overline{\cal B} = \{ b_1, \dots, b_L \} \setminus {\cal B}$ is the complementary set of ${\cal B}$.
\end{itemize}
...
Using these update equations, the search of the best split $s^*_j$ on $X_j$ can
be efficiently implemented as described in Algorithm~\ref{algo:findsplit:x_j}
and further illustrated in Figure~\ref{fig:3:split:invariant}, which completes
our specification of the tree induction procedure for numerical variables.

As we will see later in Chapter~\ref{ch:complexity}, the complexity of
Algorithm~\ref{algo:findsplit:x_j} is upper bounded by the complexity of the
sorting operation. In the context of randomized decision trees however, where
finding the very best split $s^*_j$ is not as crucial, we will show that
approximation techniques based on subsampling of the node samples or on the
discretization of the variable $X_j$ can help reduce the computational
complexity of procedure without significant impact on accuracy.
