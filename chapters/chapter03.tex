...
it expected local prediction error
\begin{equation}
Err_{{\cal L}}(t) = \mathbb{E}_{X, Y|t}\{ L(Y, \widehat{y}_t) | {\cal L} \},
\end{equation}
where the expectation for $X$ is taken over all objects $i \in \Omega$ such that $\mathbf{x}_i \in {\cal X}_t$.



% t est un modèle simpliste. On cherche à minimiser l'erreur en généralisation sur X_t
