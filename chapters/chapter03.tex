...
\end{align*}
For simplifying, let us denote $s(t) = \sum_{\mathbf{x}_i, y_i \in {\cal L}_{t}} y_i = N_t \widehat{y}_t$
...
the mean output values in $t_L$ and $t_R$ are different from the mean output
value in $t$. Contrary to classification however, this happens only in rare
circumstances, making the training error sensitive to changes in a the
posteriorio distribution, even if they are only slight.  As such, the local
resubstitution estimate is in fact a good criterion that does not exhibit
the undesirable properties that we had in classification.
...
i_R(t) = \frac{1}{N_t} \sum_{\mathbf{x}_i, y_i \in {\cal L}_t} (y_i - \widehat{y}_t)^2
\end{equation}
\end{definition}
...
% reduction of variance => error estimate is a good measure => amount to gini
...
%             Effet sur la structure des arbres générés
                ...
                % citer http://www.ise.bgu.ac.il/faculty/liorr/hbchap9.pdf  pour une comparaison des critères The Data Mining and Knowledge Discovery Handbook
