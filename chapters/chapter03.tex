...
As formulated in CART~\citep{breiman:1984}, the usual family ${\cal Q}$ of
...
samples and we are looking for the best binary split defined one of the input
...
measure $i(t)$ the local resubtitution estimate at node $t$, which in classification
is
\begin{equation}\label{eqn:impurity:error}
i(t) = 1 - p(\widehat{y}_t|t) =  1 - \max_{c \in {\cal Y}} p(c|t).
\end{equation}
As such, picking the best split would amount to pick the split that most
decreases training error. Still, in spite of its natural attractiveness,
criterion~\ref{eqn:impurity:error} has two serious defects:
\begin{enumerate}
...
\item It does not account for the structure which is learned with respect to
      the class distribution.

      For instance, splitting on $X_1$ induces a posterio class distributions
      that increase our knowledge about the overall class distribution, even if
      it does not change the predictions. In particular, we deduce from the a
      posterio class distributions that splitting on $X_1$ is likely to yield a
      tree which is simpler than if the root node had been split on
      $X_3$. As shown in Figure~\ref{fig:3:goodness}, in the former case the
      right child node is indeed already terminal, while in the latter case
      both resulting child nodes would still have to be divided further.

      This problem is largely due to the fact that the tree growing
      algorithm~\ref{algo:induction} is based on a one-step optimization
      procedure. A good impurity criterion should ideally also take into account
      the possibility of further improvement deeper in the tree.
\end{enumerate}
...
In CART, \citet{breiman:1984} identify a class of impurity functions $i(t)$
which do not have the undesirable properties of criterion~\ref{eqn:impurity:error}.

% propriété de stricte concavité => tout split s est tel que \Delta i(s, t) >= 0 => impurity is never increased by splitting
% statement du theorème Proposition 4.4 page 126
...
%             Effet sur la structure des arbres générés
                % Zighedh chap 9 intro:
                % le choix du split a une forte influence sur la structure du graphe,
                % mais finalement peu sur la qualité des prédictions
