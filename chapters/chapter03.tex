...
${\cal L}$, and then recursively partitioning the subsets of ${\cal L}$ nodes
represent into smaller subsets until convergence.  The learning procedure then
...
For now, let us broadly define an impurity measure $i(t)$ as a non-negative
function that evaluates the goodness of any node $t$, such that the closer
$i(t)$ from $0$, the purer and better the node.  Let us further assume that the
smaller $i(t)$, the better the predictions $\widehat{y}_t(\mathbf{x})$ for all
$\mathbf{x} \in {\cal L}_t$, where ${\cal L}_t$ is the subset of learning
samples $(\mathbf{x}, y) \in {\cal L}$ such that $\mathbf{x} \in {\cal X}_t$.

In those terms, a construction procedure for finding a  decision tree that
perfectly explains ${\cal L}$ consists in recursively dividing nodes into purer
and purer nodes, until all terminal nodes cannot be made purer, hence
guaranting near-optimal predictions over ${\cal L}$. The greedy assumption to
make the resulting tree as small as possible, thereby seeking for good
generalization, is then to divide each node $t$ using the split $s^*$ that
locally minimizes the impurity of the resulting child nodes.

Formally, given a learning set ${\cal L}$ of size $N$, the decrease of impurity
of a split $s$ is defined as follows:

\begin{definition}
The \emph{impurity decrease} of a split $s \in {\cal Q}$ dividing node $t$ into
a left child node $t_L$ and a right child node $t_R$ is
\begin{equation}
...
In the framework of \citet{breiman:1984}, a general greedy procedure for the
induction of decision trees is then outlined  as in
...
% minimiser impurity <=> minimiser E(phi, L)
