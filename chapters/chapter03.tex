...
to the whole input space ${\cal X}_{t_0} = {\cal X}$. It is labeled with the
split $X_1 \leq 0.7$ which divides ${\cal X}_{t_0}$ into two disjoint subsets
...
rectangles because of the nature of the splits $s_t$ dividing the nodes.
...
Given a learning set ${\cal L}$, building a decision tree therefore amounts to
determine the tree structure which produces the most accurate partition, i.e.,
the one leading to the smallest generalization error. The entire construction
then revolves around three elements:

\begin{enumerate}
\item The selection of the splits $s_k$ dividing internal nodes;
...
Implementing decision trees involves many issues that are easily overlooked
if not considered with care. The first of these issues concerns the choice
of the data structure for representing a tree-structured model.
...
paradigm, a decision tree is naturally represented as a hierarchy of high-level objects,
where each object corresponds to a node of the tree and comprises attributes
referencing its children or storing its split and value. Such a
representation would indeed constitute a correct, intuitive and flexible
implementation but may in fact not be the most appropriate when aiming for
high-performance. One of the biggest issues indeed with this approach is  that
object-oriented programming often makes extensive use of multiple levels of indirection,
which may easily impairs computing times in performance-critical applications (see
Chapter~\ref{ch:complexity} for benchmarks), e.g., by not making it possible to leverage CPU
cache mechanisms.

At the price of less abstraction and flexibility, we adopt instead in this work
a low-level representation of decision trees, thereby allowing us for a
fine-grained and complete control over memory management and CPU mechanisms. The
tree representation that we propose ....
