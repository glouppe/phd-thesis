...
\textit{split} $s_t$ taken from a set of questions ${\cal Q}$. It divides the
...
generalizes naturally to $n$-ary multi-way splits.
...
\begin{figure}
    ...
    \caption{Binary split of $t$ (left) and $N$-ary multi-way split of $t$ (right).}
    \label{fig:3:splits}
\end{figure}
...
\caption{Splitting $t_0$ on $X_1$ (above) versus splitting $t_0$ on $X_3$ (below),
         as evaluated by a strictly concave impurity function. The figure shows $i(t)$ with respect to $p(c_1|t)$.}
...
\caption{Splitting $t_0$ on $X_1$, as evaluated by the resubstitution estimate $i_R(t)$. The figure shows $i_R(t)$ with respect to $p(c_1|t)$.}
