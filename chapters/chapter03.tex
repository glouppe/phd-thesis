...
Given this definition, a general greedy procedure for the induction of binary
decision trees can then be outlined as follows:
...
\Function{BuildDecisionTree}{${\cal L}$}
    ...
    \State Create an empty queue $Q$ of \emph{open} nodes $(t, {\cal L}_t)$ ;
    \State $Q$\Call{.push}{($t_0, {\cal L}$)}
    \While{$Q$ is not empty}
        \State $t, {\cal L}_t \gets$ $Q$\Call{.pop}{~}
        \If{the stopping criterion is met for $t$}
            \State $\widehat{y}_{t} \gets$ some constant value;
        \Else
            \State Find the split that maximizes impurity decrease $$s^* = \argmax_{s \in {\cal Q}} \Delta i(s, t);$$
            ...
            \State $Q$\Call{.push}{($t_L, {\cal L}_{t_L}$)};
            \State $Q$\Call{.push}{($t_R, {\cal L}_{t_R}$)};
        \EndIf
    \EndWhile
    \State \Return $\varphi$
\EndFunction
