...
ensemble of decision trees, we will see in Chapter~\ref{ch:forest} that pruning
...
finding the split $s^* \in {\cal Q}$ that maximizes the impurity decrease $\Delta i(s^*, t)$
(Line~\ref{algo:induction:split} of Algorithm~\ref{algo:induction}).

\subsection{Families ${\cal Q}$ of splitting rules}

% on cherche une fonction X -> {L, R} (ou +)
...
\subsection{Best bi-partition on a categorical variable}

\subsection{Best bi-partition on an ordered variable}

\subsection{Approximately best bi-partition on an ordered variable}
