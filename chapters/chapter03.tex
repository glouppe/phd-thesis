...
Equation~\ref{eqn:assignment-classification} cannot be solved
...
As a corollary of Proposition~\ref{prop:any-split-reduce-classification}, the resubstitution
...
results than pre-pruning. From an interpretability point
of view, it is also a very effective framework for simplifying decision trees
and better understand the structure in the data. However, in the context of
ensemble of decision trees, we will see in Chapter~\ref{ch:forest} that (pre- or post-)pruning
is no longer required to achieve good generalization performance. For this reason,
we refer to the literature for more detailed discussion on the topic
(e.g., \citep{breiman:1984,mingers:1989,zighed:2000}).
\end{remark}
...
For a finite set ${\cal L}_t$, the number of partitions into $k$
...
the a priori class distributions induced by the split, be them close or far
from $p(c_1|t)$.
...
\begin{figure}
\centering
\includegraphics[scale=0.7]{figures/ch3_impurity_comparison.pdf}
...
preference~\citep{morgan:1973,breiman:1984}, that is the tendency to favor unbalanced
splits in which $p_L$ is close to zero or one,  often resulting in deep and uninterpretable
...
\Function{FindBestSplit}{${\cal L}_t$}
    ...
    \For{$j=1, \dots, p$}
        ...
        \If{$\Delta i(s^*_j, t) > \Delta$}
            \State $\Delta \gets \Delta i(s^*_j, t)$
            \State $s^* \gets s^*_j$
        \EndIf
    \EndFor
    \State \Return $s^*$
...
Figure~\ref{fig:3:split-ordered}, there exist exactly $|{\cal X}_{j|{\cal
L}_t}|-1$ partitions of ${\cal L}_t$ into two such non-empty subsets, that is
one for each value $v_k \in {\cal X}_{j|{\cal L}_t}$ (except for the last one
which leads to an invalid partition). In particular, if $X_j$ is locally
constant in $t$, then $|{\cal X}_{j|{\cal L}_t}|=1$ and $X_j$ cannot be used to
partition $t$. More importantly, there usually exist several thresholds $v$
producing the same partition of the node samples. If $v_k$ and $v_{k+1}$ are
two immediately consecutive values in  ${\cal X}_{j|{\cal L}_t}$, then all
splits $s^{v}_j$ for $v \in [v^k, v^{k+1}[$ actually produce the same partition
of ${\cal L}_t$ as $s^{v_k}_j$ does. In terms of impurity decrease $\Delta i$,
all of them are thus equivalent when evaluated on ${\cal L}_t$. In
generalization however, these splits may not be strictly the same since they do
not produce the same partition of ${\cal X}_t$. As a compromise, the decision
thresholds that are usually chosen to define the splits are the mid-cut-points
$v^\prime_k=\tfrac{v_k+v_{k+1}}{2}$ between consecutive values of the variable.
In practice, this is indeed a good-enough heuristic on most problems.

\begin{figure}
    \centering
    \includegraphics[scale=0.7]{figures/ch3_split_ordered.pdf}
    \caption{Binary partitions of ${\cal L}_t$ on the ordered variable $X_j$.}
    \label{fig:3:split-ordered}
\end{figure}

In this framework, finding the best split $s^*_j$ therefore amounts to examine
all partitions $\smash{s^{v^\prime_k}_j}$ and pick the one that maximizes
impurity decrease. Computationally, the exhaustive evaluation of the splits can
be carried out efficiently when done iteratively by remarking that
...
\item In classification, evaluating either $\Delta i_H(s, t)$ or $\Delta i_G(s, t)$
      ...
      for all classes $c \in {\cal Y}$. The necessary stastitics for computing
      ...
      \begin{align}
      N_{t_L}^{v^\prime_{k+1}} &= N_{t_L}^{v^\prime_{k}} + |\{ (\mathbf{x},y) | (\mathbf{x},y) \in {\cal L}^{v^\prime_{k+1}}_{t_L} \cap {\cal L}^{v^\prime_{k}}_{t_R} \}| \\
      N_{ct_L}^{v^\prime_{k+1}} &= N_{ct_L}^{v^\prime_{k}} + |\{ (\mathbf{x},y) | (\mathbf{x},y) \in {\cal L}^{v^\prime_{k+1}}_{t_L} \cap {\cal L}^{v^\prime_{k}}_{t_R}, y=c \}| \\
      N_{t_R}^{v^\prime_{k+1}} &= N_{t_R}^{v^\prime_{k}} - |\{ (\mathbf{x},y) | (\mathbf{x},y) \in {\cal L}^{v^\prime_{k+1}}_{t_L} \cap {\cal L}^{v^\prime_{k}}_{t_R} \}| \\
      N_{ct_R}^{v^\prime_{k+1}} &= N_{ct_R}^{v^\prime_{k}} - |\{ (\mathbf{x},y) | (\mathbf{x},y) \in {\cal L}^{v^\prime_{k+1}}_{t_L} \cap {\cal L}^{v^\prime_{k}}_{t_R}, y=c \}|
      \end{align}
      where ${\cal L}^{v^\prime_{k+1}}_{t_L} \cap {\cal L}^{v^\prime_{k}}_{t_R}$ are the node samples going from the right child node to the left child node when
      switching from $\smash{s^{v_k^\prime}_j}$ to $\smash{s^{v_{k+1}^\prime}_j}$.
\end{itemize}




% the goodness $\Delta i(s^v, t)$ (for $v \in
% {\cal X}_{j|t}$) of all these partitions can computed efficiently by noticing
% that $\Delta i(s^v, t)$ can be evaluated from $\Delta i(s^{v^\prime}, t)$ in a
