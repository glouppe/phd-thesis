...
splits is the set ${\cal Q}$\label{ntn:Q} of questions $s$\label{ntn:s} of the form \textit{``Is
...
into. For example, as shown in blue in Figure~\ref{fig:3:tree}, an object $\mathbf{x}=(x_1=0.2, x_2=0.7)$ falls into $t_4$
...
producing the partition which is closest to the partition engendered by $Y$ over
...
solution that fits the data, learning a decision tree from ${\cal L}$ is therefore
...
As shown by \citet{hyafil:1976}, finding the smallest tree $\varphi^*$
...
\begin{equation}\label{ntn:delta-i_t}
...
The set ${\cal S}$\label{ntn:S-all} of all possible splits $s$ is the set of all partitions of
...
binary case, splits resulting in one of the children being empty are not considered
...
the split $s^* \in {\cal Q}$\label{ntn:s-star2} is the best of the best splits defined on each input
variable. That is,
\begin{align}
...
In this section,  we describe impurity measures $i(t)$\label{ntn:i_t2} for evaluating the
...
\begin{definition}
In classification, the impurity function $i_R(t)$\label{ntn:i_t_R} based on the local resubstitution estimate
...
\caption{Comparison of $i_E(t)$, $i_H(t)$ and $i_G(t)$ with respect to $p(c_1|t)$.}
...
%In that case, $512$ random subsets ${\cal B}$ are typically drawn and evaluated.
