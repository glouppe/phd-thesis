...
The stopping criterion (Line~\ref{algo:induction:stop} of
Algorithm~\ref{algo:induction}) is usually formulated as a heuristic halting
the recursive partition if no sufficiently good split can be found.  The most
common stopping criteria can be described as follows:
...
% \begin{itemize}
% \item If the output values $y_i$ of the samples in ${\cal L}_t$ are homogeneous,
% then $t$ is set as a terminal node.

% \item If for each input variable $X_j$ the respective input values of the
% samples in ${\cal L}_t$ are all identical, then ${\cal L}_t$ cannot
% be divided further and $t$ is set as a terminal node.
% \end{itemize}
