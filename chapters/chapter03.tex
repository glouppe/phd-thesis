...
Equation~\ref{eqn:q:ordered}. The best split $s^*_j$ on $X_j$ is the best
binary partition $s^v$ of ${\cal L}_t$ into two non-empty subsets:
\begin{align*}
...
As illustrated on Figure~\ref{fig:3:split-ordered}, if $X_j$ counts $|{\cal
X}_{j|t}|$ unique values within the node samples in $t$, then there exists
$|{\cal X}_{j|t}|-1$ possible partitions of ${\cal L}_t$ into two such subsets.
Most importantly, the goodness $\Delta i(s^v, t)$ (for $v \in {\cal X}_{j|t}$) of all these partitions
can computed efficiently by noticing that $\Delta i(s^v, t)$ can be evaluated
from $\Delta i(s^{v^\prime}, t)$ in a limited number of operations.
