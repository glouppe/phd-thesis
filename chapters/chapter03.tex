...
in the a posteriori distributions, even if they are only slight.  As a result, the
...
When ${\cal Y} = \{0, 1\}$, the learning problem can be either
...
equivalent to classification trees built using the Gini index $i_G(t)$ as
impurity criterion. Indeed since $\widehat{y}_t = \tfrac{1}{N_t} \sum_{\mathbf{x}, y \in {\cal L}_t} y = p(c_2|t) = 1 - p(c_1|t)$,
\begin{align}
i_R(t) &= \frac{1}{N_t} \sum_{\mathbf{x}, y \in {\cal L}_t} (y - \widehat{y}_t)^2 \nonumber \\
...
regression trees can be implemented using criterion~\ref{eqn:impurity:variance}.
\end{remark}
