...
the red segment) and the better the split $s$. By contrast, when $s$ does not
...
\caption{Splitting $t_0$ on $X_1$ as evaluated by the resubstitution estimate.}
...
to understand why the resubstitution estimate is not an appropriate impurity
function. As long as the majority class within the child nodes $t_L$ and $t_R$
is the same than in $t$, the three red dots remain aligned in a perfect line
and therefore the gray dot overlap with $i(t)$. As a result, $\Delta i(s,t)$ is
necessarily null no matter the a priorio class distributions induced by the
