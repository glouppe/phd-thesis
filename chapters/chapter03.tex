...
model complexity, in this case by enlarging too much  the set of terminal
...
splitting, i.e., in how to formulate Line~\ref{algo:induction:stop} of
...
That is, if $y_i = y_{i^\prime}$ for all $(\mathbf{x}_i, y_i), (\mathbf{x}_{i^\prime}, y_{i^\prime}) \in {\cal L}_t$.
In particular, this is necessarily the case when $N_t = 1$.

\item When the input variables $X_j$ are each locally constant in ${\cal L}_t$. That
is, if $x_{ij} = x_{i^\prime j}$ for all $(\mathbf{x}_i, y_i), (\mathbf{x}_{i^\prime}, y_{i^\prime}) \in
{\cal L}_t$, for each input variable $X_j$. In this case, then ${\cal L}_t$
cannot be divided any further. \end{enumerate}
...
\item Set $t$ as a terminal node if it its depth is greater or equal to a threshold $d$.
($d$ is also known as \texttt{max\_depth}.)

\item \todo{Seuil sur $\Delta i(s^*, t)$ (cart, id3)}

\item \todo{Test d'independence sur chaque variable (id3)}

\end{enumerate}

In all of the above, stopping criteria are defined in terms of user-defined
hyper-parameters that need to be tuned in order to find the right
trade-off. These parameters need to be such that they are nor too strict nor too loose for the tree
to be nor too shallow nor too deep. As described in
Section~\ref{sec:2:model-selection}, chosing the appropriate parameter values
is usually performed using a dedicated model selection procedure, which
can be computationally expensive, yet usually imperative for reaching
