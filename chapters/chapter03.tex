...
\Function{FindBestSplit}{${\cal L}_t$}
    \State $\Delta = -\infty$
    \For{$j=1, \dots, p$}
        \State $s^*_j =$ \Call{FindBestSplit}{${\cal L}_t$, $X_j$}
...
$s^v_j$ of ${\cal L}_t$ into two non-empty subsets:
\begin{align*}
...
partition $t$. More importantly, there usually exist several threholds $v$
producing the same partition of the node samples. If $v_k$ and $v_{k+1}$ are
two immediately consecutives values in  ${\cal X}_{j|{\cal L}_t}$, then all
...
not produce the same partition of ${\cal X}_t$. As a result, the decision
thresholds that are usually chosen to define the splits are the mid-cutpoints
$v^\prime_k=\tfrac{v_k+v_{k+1}}{2}$ between consecutive values of the variable.
In practice, this is often a good-enough heuristic.
...
impurity decrease. Computationally, this exhaustive evaluation of all possible
splits can be done efficiently by remarking that $\smash{\Delta
i(s^{v^\prime_{k+1}}_j, t)}$ can be computed in a limited number of operations
from $\smash{\Delta i(s^{v^\prime_{k}}_j, t)}$.
