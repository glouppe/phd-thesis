...
the local generalization error with $1 - p(\widehat{y}_t | t)$, it follows:
\begin{align}
...
through probability estimates computed from class proportions derived from
${\cal L}$ reduces to the resubstitution estimate of $\varphi$
(Equation~\ref{eqn:training-error}). As a consequence, assignment
rule~\ref{eqn:assignement-classification-approx} in fact minimizes the
resubstitution estimate rather than the true
generalization error.

Finally, an important property of assignment
...
\begin{proposition}\label{prop:any-split-reduce}
For any split of a terminal node $t \in \widetilde{\varphi}$ into $t_L$ and
$t_R$, resulting in a new tree $\varphi^\prime$, $$\widehat{Err}_{\cal
...
As a corollary of Proposition~\ref{prop:any-split-reduce}, the resubstitution
...
Based on this result, we will also show in Section~\ref{sec:3:criteria} why
...
As in classification, Equation~\ref{eqn:assignment-regression} cannot be solved
without the probability distribution $P(X, Y)$. However, its solution can
approximated using estimates of $\mathbb{E}_{X, Y|t} \{ Y \}$. The simplest approximation
consists in
