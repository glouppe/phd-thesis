...
referencing its children or encoding for its split and value. Such a
...
high-performance. One of the biggest issues with this approach is  that
object-oriented programming makes extensive use of multiple levels of indirection,
which may easily impairs computing times in a high-performance context (see
Chapter~\ref{ch:complexity}), e.g., by not making it possible to leverage CPU
cache mechanisms.

At the price of less flexibility, we adopt instead a low-level
representation of decision trees, thereby allowing us for a fine and complete control
over memory management and CPU mechanisms.
