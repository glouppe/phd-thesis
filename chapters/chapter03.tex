...
Let us first consider the stopping criteria (Line~\ref{algo:induction:stop} of
Algorithm~\ref{algo:induction}) that are inherent to the iterative partition
procedure, regardless of overfitting. Node $t$ is necessarily set
as a terminal node in the following cases:

\begin{enumerate}
\item When the output values of the samples in ${\cal L}_t$ are homogeneous.
That is, if $y_i = y_j$ for all $(\mathbf{x}_i, y_i), (\mathbf{x}_j, y_j) \in {\cal L}_t$.
In particular, this is necessarily the case when $N_t = 1$.

\item When the input variables are each locally constant in ${\cal L}_t$. That
is, if $x_{ik} = x_{jk}$ for all $(\mathbf{x}_i, y_i), (\mathbf{x}_j, y_j) \in
{\cal L}_t$ and for each input variable $X_k$. In this case, then ${\cal L}_t$
cannot indeed be divided any further. \end{enumerate}
