...
the a posteriori class distributions induced by the split, be them close or far
from $p(c_1|t)$.
...
\item In regression, $i_R(t)$ can be reexpressed as the difference between the
...
Figure~\ref{fig:3:split:invariant}. As we will see later in
...
will show that approximation techniques based on subsampling of the node
...
\Function{FindBestSplit}{${\cal L}_t$, $X_j$}
    ...
    \State $i \gets 1$
    \While{$i \leq N_t$}
        \While{$i + 1 \leq N_t$ and $x_{i+1,j} \leq x_{i,j}$}
            \State $i \gets i + 1$
        \EndWhile{}
        \State $i \gets i + 1$
        \If{$i \leq N_t$}
            \State $v^\prime_{k+1} \gets \tfrac{x_{i,j} + x_{i-1,j}}{2}$
            \State Update the necessary statistics from $v^\prime_k$ to $v^\prime_{k+1}$
            \If{$\smash{\Delta i(s^{v^\prime_{k+1}}_j, t) > \Delta}$}
                \State $\smash{\Delta \gets \Delta i(s^{v^\prime_{k+1}}_j, t)}$
                \State $s^*_j \gets s^{v^\prime_{k+1}}_j$
            \EndIf{}
            \State $k \gets k+1$
        \EndIf{}
    \EndWhile{}
...
partitions in this set is $2^{L-1}-1$, which may quickly become highly probihitive to
exhaustively explore as soon as $X_j$ counts a high number $L$ of categories.

Fortunately, in binary classification, this exponential complexity can be reduced from looking at
$2^{L-1}-1$ partitions to $L-1$ partitions, thanks a theoretical result due to
\citet{fisher:1958} and \citet{breiman:1984}:
...
\end{theorem}
Intuitively, this result indicates that all those categories $b_l$
leading to high probabilities of being of class $c_1$ should be put together into
one node and the categories leading to lower probabilities into the other.
In practice, this also means that the search of the best binary split can be
performed as if $X_j$ was an ordered variable, by virtually replacing categorical values
$b_l$ with $p(c_1|t,X_j=b_l)$ and using Algorithm~\ref{algo:findsplit:x_j} to
find the best binary split on these ordered values.

Unfortunately, Theorem~\ref{thm:categorical-simplification} does not extend to
multiclass classification (i.e., when $J > 2$). In this case, an exhaustive
exploration of all $2^{L-1}-1$ splits must therefore to be performed in order
...
strategy~\citep{liaw:2002} consists in exhaustively evaluating all splits when $L
\leq 10$ and then switch to random sampling of the subsets ${\cal B} \subset \{
b_1, \dots, b_L \}$ when $L > 10$. In that case, $512$ random subsets ${\cal B}$
are typically drawn and evaluated.

In regression, given the equivalence of $i_R(t)$ with the Gini index, the same
algorithmical trick can be applied as in binary classification. In this case,
...
$\tfrac{1}{N_{l,t}} \sum_{\mathbf{x}, y \in {\cal L}_t, x_j = b_l} y$, on which
Algorithm~\ref{algo:findsplit:x_j} is then applied to find the best binary
split on these ordered values.



% encoder la variable catégorique comme une variable numérique
% si classification:
%   si J=2: théorème
%   si J>2: recherche exhaustive des splits dans les partitions de {1, ..., L}
%           si L>10, recherche aléatoire (car 2^L-1 devient trop grand)
% si régression: remplacer les valeurs de la catégorie par p(y|x=l)
