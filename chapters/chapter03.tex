...
L}_t$ into non-empty subsets are considered valid. For
example, in the binary case, splits resulting in of the two children being
empty (i.e., such that either ${\cal L}_{t_L}$ or ${\cal L}_{t_R}$ is empty) is
considered invalid since it would otherwise be impossible to assign values
to these nodes (recall that assignment rules \ref{eqn:assignement-classification-approx} and
\ref{eqn:assignement-regression-approx} are undefined for empty subsets of
${\cal L}$).
