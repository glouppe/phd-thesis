...
Starting from a single node representing the whole learning set  ${\cal L}$, a
greedy and approximate construction procedure then consists in iteratively dividing
...
split $s^*$ that locally minimizes the impurity of the resulting child nodes.
...
binary decision trees can now be described formally as outlined in
...
presents families of splitting rules along with exact and approximate
strategies for finding the best split $s^*$ (Line~\ref{algo:induction:split} of
Algorithm~\ref{algo:induction}) according to some goodness criterion, such as
those finally described in Section~\ref{sec:3:criteria}. As we will see,
assigning values to terminal nodes happens to be quite simple. The crux of the
problem is in finding good splits and in knowing when to stop splitting.
