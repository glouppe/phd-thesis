...
Let us first notice that, for a tree $\varphi$ of \textit{fixed} structure, minimizing its global generalization
error is strictly equivalent to minimizing the local generalization
error of each simplistic model in its terminal nodes. Indeed:
\begin{align}
...
expectation leaf-wise. As such, learning the best possible decion tree (of
...
$\tfrac{N_{ct}}{N_t}$ can be interpreted as the estimated probability\footnote{Lower case $p$ denotes an estimated probability while upper $P$ a theoretical probability.}
...
Similarly, let us also define the proportion $\tfrac{N_t}{N}$ as the
estimated probability $p(X \in {\cal X}_t)$ (shortly denoted $p(t)$). Plugging
this estimate into Equation~\ref{eqn:assignment-generalization} and
approximating the inner expectation with $1 - p(\widehat{y}_t | t)$ as above,
we have
\begin{align}
\sum_{t \in \widetilde{\varphi}} p(t) (1 - p(\widehat{y}_t | t)) &= \frac{1}{N} \sum_{t \in \widetilde{\varphi}} N_t - N_{{\widehat{y}_t}t} \nonumber \\
    &= \frac{1}{N} \sum_{t \in \widetilde{\varphi}} \sum_{\mathbf{x}_i, y_i \in {\cal L}_t} 1(y_i \neq \widehat{y}_t) \nonumber \\
    &= \frac{1}{N} \sum_{\mathbf{x}_i, y_i \in {\cal L}} 1(y_i \neq \varphi(t))
\end{align}
In other words, minimizing Equation~\ref{eqn:assignment-generalization}
through probability estimates computed from class proportions in the learning
set ${\cal L}$ is in fact equivalent to minimizing the resubstitution estimate of $\varphi$.

% équivalent à minimizer R(T) globalement
% splitter t réduit R(T) => déveloper totalement conduit à des arbres parfaits
